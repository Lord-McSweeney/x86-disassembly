...
use crate::op::{
    AddressRegisters, Bits, Op, OpCode, OpPrefix, Operand, ParseError, Register, SegmentRegister,
    SmallRegister,
};
...
impl<'data> X86ByteStream<'data> {
    ...
    fn read_16_or_32bit_constant(&mut self, bits: Bits) -> Result<Operand, ParseError> {
        Ok(match bits {
    ...
    fn read_16bit_mod0_operand_8bit_result(&mut self, rm: u8) -> Result<Operand, ParseError> {
        match rm {
    ...
    fn read_16bit_mod1_operand_8bit_result(&mut self, rm: u8) -> Result<Operand, ParseError> {
    ...
    fn read_16bit_mod2_operand_8bit_result(&mut self, rm: u8) -> Result<Operand, ParseError> {
    ...
    fn read_32bit_mod0_operand_8bit_result(&mut self, rm: u8) -> Result<Operand, ParseError> {
        match rm {
            0..=3 | 6..=7 => {
            ...
            5 => Ok(Operand::AbsoluteAddress32Byte {
    ...
    fn read_32bit_mod1_operand_8bit_result(&mut self, rm: u8) -> Result<Operand, ParseError> {
        match rm {
            0..=3 | 5..=7 => {
    ...
    fn read_32bit_mod2_operand_8bit_result(&mut self, rm: u8) -> Result<Operand, ParseError> {
        match rm {
            0..=3 | 5..=7 => {
                // EAX, ECX, EDX, EBX, EBP, ESI, or EDI
                let register = Register::from_byte(rm, Bits::Bit32);
    ...
    fn read_32bit_mod0_operand_16bit_result(
        ...
        match rm {
            ...
            5 => Ok(Operand::AbsoluteAddress32WordOrDword {
                address: self.read_u32()?,
                bits: operand_bits,
            }),
            _ => unreachable!(),
    ...
    fn read_special_op_operand_8bit_result(
        ...
        Ok(match modrm.0 {
            0 => match address_bits {
                Bits::Bit16 => self.read_16bit_mod0_operand_8bit_result(modrm.2)?,
                Bits::Bit32 => self.read_32bit_mod0_operand_8bit_result(modrm.2)?,
            },
            1 => match address_bits {
                Bits::Bit16 => self.read_16bit_mod1_operand_8bit_result(modrm.2)?,
                Bits::Bit32 => self.read_32bit_mod1_operand_8bit_result(modrm.2)?,
            },
            2 => match address_bits {
                Bits::Bit16 => self.read_16bit_mod2_operand_8bit_result(modrm.2)?,
                Bits::Bit32 => self.read_32bit_mod2_operand_8bit_result(modrm.2)?,
            },
            3 => Operand::SmallRegister {
                register: SmallRegister::from_byte(modrm.2),
            },
            _ => unreachable!(),
        })
    }
    ...
    fn read_special_op_operand_16_or_32bit_result(
        ...
        Ok(match modrm.0 {
            0 => match address_bits {
                Bits::Bit16 => self.read_16bit_mod0_operand_16bit_result(modrm.2, operand_bits)?,
                Bits::Bit32 => self.read_32bit_mod0_operand_16bit_result(modrm.2, operand_bits)?,
            },
            1 => match address_bits {
                Bits::Bit16 => self.read_16bit_mod1_operand_16bit_result(modrm.2, operand_bits)?,
                Bits::Bit32 => self.read_32bit_mod1_operand_16bit_result(modrm.2, operand_bits)?,
            },
            2 => match address_bits {
                Bits::Bit16 => self.read_16bit_mod2_operand_16bit_result(modrm.2, operand_bits)?,
                Bits::Bit32 => self.read_32bit_mod2_operand_16bit_result(modrm.2, operand_bits)?,
            },
            3 => Operand::Register {
    ...
    fn read_regpair_general(
        ...
        match modrm.0 {
            0 => match address_bits {
                Bits::Bit16 => {
                    let second_mem =
                        self.read_16bit_mod0_operand_16bit_result(modrm.2, operand_bits)?;
                ...
                Bits::Bit32 => {
                    let second_mem =
                        self.read_32bit_mod0_operand_16bit_result(modrm.2, operand_bits)?;
            ...
            1 => match address_bits {
                Bits::Bit16 => {
                    let second_mem =
                        self.read_16bit_mod1_operand_16bit_result(modrm.2, operand_bits)?;
                ...
                Bits::Bit32 => {
                    let second_mem =
                        self.read_32bit_mod1_operand_16bit_result(modrm.2, operand_bits)?;
            ...
            2 => match address_bits {
                Bits::Bit16 => {
                    let second_mem =
                        self.read_16bit_mod2_operand_16bit_result(modrm.2, operand_bits)?;
                ...
                Bits::Bit32 => {
                    let second_mem =
                        self.read_32bit_mod2_operand_16bit_result(modrm.2, operand_bits)?;
        ...
        let op = opcode.and_then(|mut opcode| {
            ...
            loop {
                match opcode {
                    ...
                    0x3E => prefixes.push(OpPrefix::Ds),
            ...
            let op_info = match opcode {
                0x00 => {
                    ...
                    let first_operand =
                        stream.read_special_op_operand_8bit_result(modrm, address_bits)?;
                ...
                0x01 => {
                    let operands =
                        stream.read_regpair_general(operand_bits, address_bits, false)?;
                ...
                0x05 => {
                    ...
                    (OpCode::Add, vec![ax_reg, second_operand])
                }
                0x06 => (
                    OpCode::Push,
                    vec![Operand::SegmentRegister {
                ...
                0x0F => {
                    ...
                    match second_byte {
                            ...
                            match address_bits {
                                Bits::Bit16 => {
                                    ...
                                    jump_targets.push(
                                        stream.pos as isize
                                            + offset as isize
                                            + load_offset as isize,
                                    );

                                    (instr, vec![Operand::RelativeOffset16 { offset }])
                                }
                                Bits::Bit32 => {
                                    ...
                                    (instr, vec![Operand::RelativeOffset32 { offset }])
                                }
                            }
                        }
                        0xB6 => {
                        ...
                        0xB7 => {
                            ...
                            let first_operand = Operand::Register {
                                register: Register::from_byte(modrm.1, Bits::Bit32),
                            };

                            let second_operand = stream
                                .read_special_op_operand_16_or_32bit_result(
                                    modrm,
                                    Bits::Bit16,
                                    address_bits,
                                )?;
                        ...
                        0xBE => {
                            ...
                            (OpCode::Movsx, vec![first_operand, second_operand])
                        }
                ...
                0x3D => {
                    ...
                    (OpCode::Cmp, vec![ax_reg, second_operand])
                }
                0x40..=0x47 => {
                ...
                0x60 => (OpCode::PushA, vec![]),
                0x61 => (OpCode::PopA, vec![]),
                0x68 => {
                ...
                0x6A => (
                    OpCode::Push,
                    vec![Operand::Constant8 {
                        value: stream.read_u8()?,
                    }],
                ),
                0x6C => (OpCode::InSb, vec![]),
                0x6E => (OpCode::OutSb, vec![]),
                0x6F => (OpCode::OutSw, vec![]),
                0x72 => {
                    let offset = stream.read_i8()?;

                    jump_targets.push(stream.pos as isize + offset as isize + load_offset as isize);

                    (OpCode::Jb, vec![Operand::RelativeOffset8 { offset }])
                }
                0x73 => {
                    let offset = stream.read_i8()?;
                    ...
                    (OpCode::Jae, vec![Operand::RelativeOffset8 { offset }])
                }
                0x74 => {
                    let offset = stream.read_i8()?;
                    ...
                    (OpCode::Jz, vec![Operand::RelativeOffset8 { offset }])
                }
                0x75 => {
                    let offset = stream.read_i8()?;
                    ...
                    (OpCode::Jnz, vec![Operand::RelativeOffset8 { offset }])
                }
                0x76 => {
                    let offset = stream.read_i8()?;
                    ...
                    (OpCode::Jbe, vec![Operand::RelativeOffset8 { offset }])
                }
                0x77 => {
                    let offset = stream.read_i8()?;
                    ...
                    (OpCode::Ja, vec![Operand::RelativeOffset8 { offset }])
                }
                0x79 => {
                    let offset = stream.read_i8()?;
                    ...
                    (OpCode::Jns, vec![Operand::RelativeOffset8 { offset }])
                }
                0x7C => {
                    let offset = stream.read_i8()?;
                    ...
                    (OpCode::Jl, vec![Operand::RelativeOffset8 { offset }])
                }
                0x7D => {
                    let offset = stream.read_i8()?;
                    ...
                    (OpCode::Jge, vec![Operand::RelativeOffset8 { offset }])
                }
                0x7E => {
                    let offset = stream.read_i8()?;
                    ...
                    (OpCode::Jle, vec![Operand::RelativeOffset8 { offset }])
                }
                0x7F => {
                    let offset = stream.read_i8()?;
                    ...
                    (OpCode::Jg, vec![Operand::RelativeOffset8 { offset }])
                }
                0x80 => {
                    let modrm = stream.read_modrm()?;
                ...
                0x8D => {
                    let operands = stream.read_regpair_general(operand_bits, address_bits, true)?;
                ...
                0xA4 => (OpCode::MovSb, vec![]),
                0xA5 => match operand_bits {
                    Bits::Bit16 => (OpCode::MovSw, vec![]),
                    Bits::Bit32 => (OpCode::MovSd, vec![]),
                },
                0xA6 => (OpCode::CmpSb, vec![]),
                0xAC => (OpCode::LodSb, vec![]),
                0xAD => (OpCode::LodSw, vec![]),
                0xAE => (OpCode::ScaSb, vec![]),
                0xB0..=0xB7 => {
                ...
                0xC7 => {
                    ...
                    (OpCode::Mov, vec![operand, second_operand])
                }
                0xC9 => (OpCode::Leave, vec![]),
                0xCB => (OpCode::RetF, vec![]),
                0xCC => (OpCode::Int, vec![Operand::Constant8 { value: 3 }]),
                0xCD => (
                    OpCode::Int,
                    vec![Operand::Constant8 {
                        value: stream.read_u8()?,
                ...
                0xD1 => {
                    ...
                    let constant_1 = Operand::Constant8 { value: 1 };
                ...
                0xE0 => {
                    ...
                    (OpCode::LoopNz, vec![Operand::RelativeOffset8 { offset }])
                }
                0xE2 => {
                    let offset = stream.read_i8()?;
                    ...
                    (OpCode::Loop, vec![Operand::RelativeOffset8 { offset }])
                }
                0xE3 => {
                    let offset = stream.read_i8()?;
                    ...
                    (OpCode::Jcxz, vec![Operand::RelativeOffset8 { offset }])
                }
                0xE8 => match address_bits {
                    Bits::Bit16 => {
                        let offset = stream.read_i16()?;

                        jump_targets
                            .push(stream.pos as isize + offset as isize + load_offset as isize);

                        (OpCode::Call, vec![Operand::RelativeOffset16 { offset }])
                    }
                    Bits::Bit32 => {
                        let offset = stream.read_i32()?;
                        ...
                        (OpCode::Call, vec![Operand::RelativeOffset32 { offset }])
                    }
                },
                0xE9 => match address_bits {
                    Bits::Bit16 => {
                        let offset = stream.read_i16()?;
                        ...
                        (OpCode::Jmp, vec![Operand::RelativeOffset16 { offset }])
                    }
                    Bits::Bit32 => {
                        let offset = stream.read_i32()?;
                        ...
                        (OpCode::Jmp, vec![Operand::RelativeOffset32 { offset }])
                    }
                },
                0xEA => match address_bits {
                    Bits::Bit16 => {
                        ...
                        (
                            OpCode::Jmp,
                            vec![Operand::AbsoluteConstantSegmentedOffset16 { segment, offset }],
                        )
                    }
                    Bits::Bit32 => {
                        ...
                        (
                            OpCode::Jmp,
                            vec![Operand::AbsoluteConstantSegmentedOffset32 { segment, offset }],
                        )
                    }
                },
                0xEB => {
                    ...
                    let jump_result = (OpCode::Jmp, vec![Operand::RelativeOffset8 { offset }]);
